...
function ChangeBadge({ curr, prev }: { curr: number; prev: number | null | undefined }) {
    ...
    // No comparison possible when previous period had zero activity
                    ...
                    {data.map(d => {
                        ...
                        return (
                            <div key={d.date} className="flex-1 flex flex-col items-center justify-end group relative">
                                ...
                                {/* Best day star */}
                                {isBest && (
                                    <span className="absolute -top-5 text-[10px]">⭐</span>
                                )}
                                <div
...
export default function AnalyticsClient({ periodData, dailyRevenue, peakHours, peakWeekdays, bestDay, summary }: Props) {
    ...
    const { curr, prev, topItems, statusCounts, dateLabel, prevLabel } = periodData[period];
    ...
    const statCards: { label: string; value: string; currVal: number; prevVal: number | undefined; prevFmt?: string; icon: React.ReactNode }[] = [
        {
    ...
    return (
        <div className="space-y-8">
            ...
            <div className="flex flex-wrap items-start justify-between gap-4">
                ...
                <button onClick={() => exportCSV(dailyRevenue)}
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                {[
                    ...
                    <div key={card.label} className={`rounded-2xl border bg-white/5 p-4 flex items-center gap-3 ${(card as { warn?: boolean }).warn ? "border-red-500/20" : "border-white/10"}`}>
                        ...
                        <div className="min-w-0">
                            ...
                            <div className={`font-black text-base leading-none ${(card as { warn?: boolean }).warn && cancelRate > 10 ? "text-red-400" : "text-white"}`}>{card.value}</div>
