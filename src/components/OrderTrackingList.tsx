...
export default function OrderTrackingList({ initialOrders }: { initialOrders: any[] }) {
    ...
    const handlePageChange = (newPage: number) => {
        ...
        // Scroll smoothly to the top of the order history parent section
            ...
            {totalPages > 1 && (
                <div className="flex items-center justify-between p-4 bg-white/5 border border-white/10 rounded-2xl backdrop-blur-md">
                    <button
                        ...
                        className="flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-gray-500 hover:text-orange-500 disabled:opacity-30 disabled:hover:text-gray-500 transition"
                    >
                    ...
                    <div className="flex flex-col items-center gap-1">
                        <span className="text-[10px] font-black text-white uppercase tracking-widest">
                            Page {currentPage} <span className="text-gray-500 mx-1">/</span> {totalPages}
                        </span>
                        <div className="flex gap-1">
                            {[...Array(totalPages)].map((_, i) => (
                                <div
                                    key={i}
                                    className={`h-1 rounded-full transition-all duration-300 ${i + 1 === currentPage ? "w-4 bg-orange-500" : "w-1 bg-white/10"}`}
                                />
                            ))}
                        </div>
                    </div>
