...
export default function CartDrawer() {
    ...
    useEffect(() => {
        ...
        const fetchWaitTime = async () => {
            try {
                const res = await fetch("/api/orders/wait-time");
                if (res.ok) {
                    const data = await res.json();
                    setWaitTime(data.displayTime);
                }
            } catch (err) {
    ...
    const handleApplyPromo = async () => {
        ...
        try {
            const res = await fetch("/api/promo/validate", {
                ...
                body: JSON.stringify({ code: promoCode, subtotal: subtotalCents }),
            });
            const data = await res.json();
                        ...
                        >
                            <div className="flex h-full flex-col">
                                <div className="flex items-center justify-between p-6 border-b border-white/10">
                                                ...
                                                <div className="space-y-4 pt-6 border-t border-white/5">
                                                    ...
                                                    <div className="space-y-2">
                                                        <input
                                                            ...
                                                            className={`w-full bg-black/40 border rounded-xl px-4 py-2.5 text-sm outline-none transition placeholder:text-gray-700 ${
                                                                fieldErrors.name
                                                        ...
                                                        />
                                                        {fieldErrors.name && (
                                                            <p className="mt-1 text-[11px] text-red-400 font-medium">{fieldErrors.name}</p>
                                                        )}
                                                        ...
                                                        <div className="grid grid-cols-2 gap-2">
                                                            ...
                                                            <input
                                                                ...
                                                                placeholder="Phone"
                                                                className={`w-full bg-black/40 border rounded-xl px-4 py-2.5 text-sm outline-none transition placeholder:text-gray-700 ${
                                                                    fieldErrors.phone ? "border-red-500/70" : "border-white/10"
                                                                }`}
                                                        ...
                                                        <textarea
                                                            ...
                                                            className="w-full bg-black/40 border border-white/10 rounded-xl px-4 py-3 text-xs text-white outline-none focus:border-orange-500/50 transition placeholder:text-gray-700 resize-none h-16"
                                    ...
                                    {items.length > 0 && (
                                        <div className="p-6 space-y-4 bg-white/5 border-t border-white/10 backdrop-blur-3xl">
                                            {/* Promo Code Section */}
                                            <div className="space-y-2">
                                                <h4 className="font-black text-[9px] uppercase tracking-[0.2em] text-gray-500 italic">Promo Code</h4>
                                                {appliedPromo ? (
                                                    <div className="flex items-center justify-between bg-orange-500/10 border border-orange-500/20 rounded-xl px-4 py-2.5">
                                                        <div className="flex items-center gap-2">
                                                            ...
                                                            <span className="font-black text-[10px] uppercase tracking-widest text-white">{appliedPromo.code}</span>
                                                            <span className="text-[9px] font-bold text-orange-400">
                                                                ({appliedPromo.discountType === "PERCENTAGE" ? `${appliedPromo.discountValue}%` : `$${(appliedPromo.discountValue / 100).toFixed(2)}`} OFF)
                                                            </span>
                                                        </div>
                                                        <button onClick={removePromo} className="text-gray-500 hover:text-white transition" title="Remove Promo Code" aria-label="Remove Promo Code">
                                                            <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" />
                                                            </svg>
                                                        </button>
                                                    ...
                                                    <div className="flex gap-2">
                                                        <input
                                                            ...
                                                            className={`flex-1 bg-black/40 border rounded-xl px-4 py-2 text-xs outline-none transition placeholder:text-gray-800 uppercase ${promoError ? 'border-red-500/50' : 'border-white/10 focus:border-orange-500/30'}`}
                                                            value={promoCode}
                                                        ...
                                                        <button 
                                                            ...
                                                            className="px-4 py-2 bg-white/5 border border-white/10 rounded-xl text-[9px] font-black uppercase tracking-widest hover:bg-white/10 transition disabled:opacity-50"
                                                        >
                                                            {isValidatingPromo ? "..." : "Apply"}
                                                        </button>
                                                    </div>
                                                )}
                                                {promoError && <p className="text-[10px] text-red-400 font-medium px-1">{promoError}</p>}
                                            </div>

                                            {/* Wait Time Estimation */}
                                            {waitTime && isTruckOpen && (
                                                <div className="bg-orange-500/5 border border-orange-500/10 rounded-2xl p-4 flex items-center justify-between group">
                                                    <div className="flex items-center gap-3">
                                                        <div className="w-10 h-10 rounded-xl bg-orange-500/10 flex items-center justify-center text-xl shadow-inner group-hover:scale-110 transition-transform">
                                                            ⏱️
                                                        </div>
                                                        <div>
                                                            <p className="text-[10px] font-black uppercase tracking-[0.2em] text-orange-500/70 italic">Estimated Prep Time</p>
                                                            <p className="text-sm font-bold text-white tracking-tight">Ready in {waitTime}</p>
                                                        </div>
                                                    </div>
                                                    <div className="text-[9px] font-bold text-gray-600 uppercase tracking-widest bg-white/5 px-2 py-1 rounded-lg">
                                                        Live Load
                                                    </div>
                                                </div>
                                            )}
                                            ...
                                            <div className="space-y-1.5 pt-4 border-t border-white/5">
                                                ...
                                                {appliedPromo && (
                                                    <div className="flex items-center justify-between text-[10px] font-bold text-orange-500 uppercase tracking-widest">
                                                ...
                                                <div className="flex items-center justify-between text-[10px] font-bold text-gray-500 uppercase tracking-widest">
                                                    <span>Tax (6.35%)</span>
                                                    <span>${(Math.round((subtotalCents - discountCents) * 0.0635) / 100).toFixed(2)}</span>
                                                </div>
                                                <div className="flex items-center justify-between pt-2">
                                                    ...
                                                    <span className="text-3xl font-black italic tracking-tighter text-orange-500 shadow-orange-500/10 shadow-2xl">
                                                        ${((totalCents + Math.round((subtotalCents - discountCents) * 0.0635)) / 100).toFixed(2)}
                                                    </span>
                                                </div>
                                            </div>
                                            ...
                                            {!isTruckOpen && (
                                                <div className="flex items-center gap-3 bg-white/5 border border-white/10 rounded-2xl px-4 py-3">
                                                    ...
                                                    <div>
                                                        ...
                                                        <p className="text-[10px] text-gray-500 mt-0.5">Orders accepted during serving hours only</p>
                                                    </div>
                                                </div>
                                            )}
